...
export async function getAllGrapeReportsWithSlug() {
  const data = await client.fetch(`*[_type == "grapeReport"]{ 'slug': slug.current }`)
  return data
}
...
export async function getGrapeReportsAndMoreGrapeReports(slug, preview) {
  ...
  return { grapeReport, moreGrapeReports: getUniqueGrapeReports(moreGrapeReports) }
}
