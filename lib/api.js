...
const postFields = `
  ...
  'label': label->{name},
  'variety': variety->{name, 'image': image.asset->url},
  'stems': stems->{name},
...
export async function getAllPostsWithSlug() {
  const data = await client.fetch(`*[_type == "post" && defined(title) && defined(label) && defined(lot)]{ 'slug': slug.current }`);
  return data;
}
...
export async function getAllPostsForHome(preview) {
  const results = await getClient(preview)
    .fetch(`*[_type == "post" && defined(lot)] | order(publishedAt desc){
      ${postFields}
      body
    }`);
